...
class AddPermitRequester(val permit: Permit) : Runnable {
    var TAG = AddPermitRequester::class.java.name

    override fun run() {
        var czResponse: CZResponse<ResponseBean<Any>>? = null
        czResponse = ApiHelper.performAddPermit(permit) as CZResponse<ResponseBean<Any>>
        if (czResponse != null && czResponse!!.getResponse() != null) {
            if (czResponse!!.getResponse().getStatus()) {
                EventBus.getDefault().post(EventObject(EventConstant.ADD_PERMIT_SUCCESS, czResponse.response.message))
            } else {
                EventBus.getDefault().post(EventObject(EventConstant.ADD_PERMIT_ERROR, czResponse!!.getResponse().getMessage()))
            }
        } else {
